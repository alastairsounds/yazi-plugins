...
--- Fetcher entry point. Runs off the main thread; counts @todo occurrences
--- with `rg` and stores results with `add`.
---
--- Must return a coroutine (`ya.co`). The coroutine yields `(file, result)`
--- for each file. Yield `{ retry = true }` to skip a file for now. Yazi
--- retries it on the next fetch.
local function fetch(_, job)
	return ya.co(function()
		if #job.files > 0 and tostring(job.files[1].url):match("^search://") then
			for _, file in ipairs(job.files) do coroutine.yield(file, { retry = true }) end
			return
		end
		...
		if git_only and #job.files > 0 then
			...
			if not in_git_repo(parent) then
				for _, file in ipairs(job.files) do coroutine.yield(file, { retry = true }) end
				return
			end
		end
